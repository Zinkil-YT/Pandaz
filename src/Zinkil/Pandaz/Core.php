...
use Zinkil\Pandaz\commands\{KickAllCommand, AntiCheatCommand, ResetStatsCommand, SetClanTagCommand, ReplyCommand, DuelCommand, ForceKitCommand, KitCommand, NickCommand, MuteCommand, TempBanCommand, PermBanCommand, CoordsCommand, MessagesCommand, VanishCommand, KillCommand, PartyCommand, FlyCommand, StoPandazommand, ExecCommand, TpallCommand, AliasCommand, PingCommand, ForceRankCommand, ManageCommand, OnlineCommand, DisguiseCommand, MuteChatCommand, WhoCommand, StaffCommand, HubCommand, TestCommand, WhisperCommand, AnnounceCommand, FreezeCommand, RankCommand, GamemodeCommand};
use Zinkil\Pandaz\listeners\{ServerListener, AntiCheatListener, WorldListener, ItemListener, PlayerListener, PvPUtilsListener};
use Zinkil\Pandaz\tasks\{FlagsTask, StatusTask, ParticleTask, PlayerTask, VoteAccessTask, TemporaryBansTask, MutesTask, DuelTask, CombatTask, TemporaryRankTask, DatabaseTask, SetDayTask, DropsTask, MotdTask, PingTask, FloatingTextTask, VanishTask, NameTagTask, BroadcastTask, CombatCheckTask};
use Zinkil\Pandaz\handlers\{ArenaHandler, DuelHandler, ClickHandler, ScoreboardHandler, PermissionHandler, DatabaseHandler};
use Zinkil\Pandaz\entities\{PotionEntity, FastPotion, DefaultPotion, Pearl, Hook};
...
use Zinkil\Pandaz\multiver\PMPlayer;
...
class Core extends PluginBase{
	...
	public function setCommands(){
		...
		$map->register("stop", new StoPandazommand($this));
