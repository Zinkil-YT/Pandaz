...
use Zinkil\Pandaz\forms\{SimpleForm, ModalForm, CustomForm};
...
class StaffUtils{
	...
	public function teleportForm(Player $player):void{
		$form=new SimpleForm(function(Player $player, $data=null):void{
			switch($data){
				...
				break;
				case "fist":
				$player->sendTo(5, false, false);
				break;
				case "resistance":
				$player->sendTo(9, false, false);
				break;
				case "sumoffa":
				$player->sendTo(11, false, false);
				break;
				case "knockbackffa":
				$player->sendTo(12, false, false);
				break;
				case "rushffa":
				$player->sendTo(13, false, false);
				break;
				case "offline":
		...
		$fist=Core::getInstance()->getServer()->getLevelByName("fist");
		$resistance=Core::getInstance()->getServer()->getLevelByName("resistance");
		$sumoffa=Core::getInstance()->getServer()->getLevelByName("sumoffa");
		$knockbackffa=Core::getInstance()->getServer()->getLevelByName("knockbackffa");
		$rushffa=Core::getInstance()->getServer()->getLevelByName("rushffa");
		...
		if(!Core::getInstance()->getServer()->isLevelLoaded("lobby")){
			$details4="Players: 0 Status: §cOffline";
			$c4="offline";
			}else{
				$details4="Players: ".count($lobby->getPlayers())." Status: §aOnline";
				$c4="lobby";
		}
		if(!Core::getInstance()->getServer()->isLevelLoaded("fist")){
			$details5="Players: 0 Status: §cOffline";
			$c5="offline";
			}else{
				$details5="Players: ".count($fist->getPlayers())." Status: §aOnline";
				$c5="fist";
		}
		if(!Core::getInstance()->getServer()->isLevelLoaded("resistance")){
			$details6="Players: 0 Status: §cOffline";
			$c6="offline";
			}else{
				$details6="Players: ".count($resistance->getPlayers())." Status: §aOnline";
				$c6="resistance";
		}
		if(!Core::getInstance()->getServer()->isLevelLoaded("sumoffa")){
			$details7="Players: 0 Status: §cOffline";
			$c7="offline";
			}else{
				$details7="Players: ".count($sumoffa->getPlayers())." Status: §aOnline";
				$c7="sumoffa";
		}
		if(!Core::getInstance()->getServer()->isLevelLoaded("knockbackffa")){
			$details8="Players: 0 Status: §cOffline";
			$c8="offline";
			}else{
				$details8="Players: ".count($knockbackffa->getPlayers())." Status: §aOnline";
				$c8="knockbackffa";
		}
		if(!Core::getInstance()->getServer()->isLevelLoaded("rushffa")){
			$details9="Players: 0 Status: §cOffline";
			$c9="offline";
			}else{
				$details9="Players: ".count($rushffa->getPlayers())." Status: §aOnline";
				$c9="rushffa";
		}
		...
		$form->addButton("SumoFFA\n".$details7, 0, "textures/ui/worldsIcon", $c7);
		$form->addButton("KnockBackPvP\n".$details8, 0, "textures/ui/worldsIcon", $c8);
		$form->addButton("RushFFA\n".$details9, 0, "textures/ui/worldsIcon", $c9);
	...
	public function staffPortalForm(Player $player):void{
		...
		if($player->hasPermission("Pandaz.command.tban")){
