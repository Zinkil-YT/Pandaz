...
namespace Zinkil\Pandaz\multiver;
...
class MultiVersion extends Player{
    ...
    public function broadcastMovement(bool $teleport = false): void
    {
        ...
        /** @var CorePlayer[] $viewers */
        $viewers = $this->getViewers();

        // Broadcasts the packet to 1.16.220 & below players.
        Utils::broadcastPacketToViewers($this, $pk, function(CorePlayer $player) {
            return strpos($player->getVersion(), "1.16.220") === false;
        }, $viewers);

        /** @var CorePlayer[] $viewers */
        $viewers = array_filter($viewers, function(CorePlayer $viewer) {
            return strpos($viewer->getVersion(), "1.16.220") !== false;
        });
