...
use Zinkil\Pandaz\party\Party;
use Zinkil\Pandaz\party\PartyManager;
...
class CorePlayer extends Player{
	...
	public function setTagged($value=true){
		if($value){
			$this->plugin->taggedPlayer[Utils::getPlayerName($this)]=11;
			}else{
	...
	public function handleLevelSoundEvent(LevelSoundEventPacket $packet):bool{
		...
		Utils::broadcastPacketToViewers($this, $packet, function(Player $player, DataPacket $packet){
			if($player instanceof CorePlayer and $packet instanceof LevelSoundEventPacket){
	...
	public function sendTo(int $loc, bool $kit=false, bool $title=false, $leader=true){
		switch($loc){
			...
			$z=0;
			$yaw=mt_rand(0, 180);
			$world=$this->plugin->getServer()->getLevelByName("knockbackffa");
			$this->teleport(new Location($x, $y, $z, $yaw, 0, $world));
			$this->setPlayerLocation($loc);
			if($kit===true){
			...
			case 13:
				...
				$z=0;
				$yaw=mt_rand(0, 180);
				$world=$this->plugin->getServer()->getLevelByName("rushffa");
				$this->teleport(new Location($x, $y, $z, $yaw, 0, $world));
				$this->setPlayerLocation($loc);
				if($kit===true){
					Kits::sendKit($this, "rushffa");
				}
				if($title===true){
					$this->sendMessage("§6Kits §7»§r§f You Selected §bRushFFA");
				}
				$this->plugin->getScoreboardHandler()->sendMainScoreboard($this);
				break;
			default:
			...
			if($damager instanceof Player){
				if($this->plugin->getDuelHandler()->isInPartyDuel($damager)){
					...
					switch($this->getLevel()){
						...
						$this->attackTime=7;
						break;
						case $this->plugin->getServer()->getLevelByName("knockbackffa");
						$this->attackTime=9;
						break;
						case $this->plugin->getServer()->getLevelByName("rushffa");
						$this->attackTime=9;
						break;
					}
				}
