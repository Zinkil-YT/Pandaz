...
use Zinkil\Pandaz\duels\groups\DuelSpectator;
use Zinkil\Pandaz\tasks\onetime\OpenArenaTask;
use Zinkil\Pandaz\tasks\onetime\LoadLevelTask;
...
class DuelGroup{
	...
	private function endDuel(bool $endPrematurely=false, bool $disablePlugin=false, $win=false):void{
		...
		if($player instanceof CorePlayer) $player->setTagged(false);
		if($opponent instanceof CorePlayer) $opponent->sendTo(0, true);
	...
	public function initializePlayers(int $type):void{
		switch($type){
			...
			if($this->isOpponentOnline()){
				...
				$opponent->addEffect(new EffectInstance(Effect::getEffect(Effect::BLINDNESS), 999999, 5, false));
				$opponent->setFood(20);
			...
			if($this->isPlayerOnline()){
				...
				$player->addEffect(new EffectInstance(Effect::getEffect(Effect::BLINDNESS), 999999, 5, false));
				$player->setFood(20);
				$player->setHealth(20);
			}
			break;
	...
	public function initializeWin($player):void{
		if(Utils::isPlayer($player)){
			if(!is_null($player)){
				...
				$ranked=$this->isRanked();
				if($ranked===true){
					if($player instanceof CorePlayer) Utils::matchOutcome($player, 0, true);
				}else{
					if($player instanceof CorePlayer) Utils::matchOutcome($player, 0, false);
				}
			}
		}
	}
	...
	public function initializeLoss($player):void{
		if(Utils::isPlayer($player)){
			if(!is_null($player)){
				...
				if($ranked===true){
					if($player instanceof CorePlayer) Utils::matchOutcome($player, 1, true);
				}else{
					if($player instanceof CorePlayer) Utils::matchOutcome($player, 1, false);
				}
			}
		}
	}
