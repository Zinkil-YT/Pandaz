...
class ItemListener implements Listener{
	...
	public function warpForm(Player $player):void{
		...
		$knockbackffa=$this->plugin->getServer()->getLevelByName("knockbackffa");
		$rushffa=$this->plugin->getServer()->getLevelByName("rushffa");
		...
		if(!$this->plugin->getServer()->isLevelLoaded("rushffa")){
			$count9="§cOffline";
			$c9="offline";
		}else{
			$count9="Playing: §b".count($rushffa->getPlayers());
			$c9="rushffa";
		}
	...
	public function nodebuffForm(Player $player):void{
		...
		if(!$this->plugin->getServer()->isLevelLoaded("nodebuff")){
			...
			$count1="Playing: §b".count($normal->getPlayers());
			$c1="normal";
		}
		if(!$this->plugin->getServer()->isLevelLoaded("nodebuff-low")){
			...
			$count2="Playing: §b".count($lowkb->getPlayers());
			$c2="lowkb";
		}
	...
	public function rankedForm(Player $player):void{ 
		...
		$form->addButton("§l§bBuildUHC\n§r§8Queued: §b".$builduhcqueued."§8 Matches: §b".$builduhcplaying, 0, "textures/items/bucket_lava", "builduhc");
		$form->addButton("§l§bDiamond\n§r§8Queued: §b".$diamondqueued."§8 Matches: §b".$diamondplaying, 0, "textures/ui/strength_effect", "diamond");
		if($this->plugin->getDuelHandler()->isPlayerInQueue($player)){
			$result=$this->plugin->getDuelHandler()->getQueuedPlayer($player);
			if(is_null($result)) return;
			$ranked=$result->isRanked();
	...
	public function unrankedForm(Player $player):void{ 
		...
		$form->addButton("§l§bBuildUHC\n§r§8Queued: §b".$builduhcqueued."§8 Matches: §b".$builduhcplaying, 0, "textures/items/bucket_lava", "builduhc");
		$form->addButton("§l§bDiamond\n§r§8Queued: §b".$diamondqueued."§8 Matches: §b".$diamondplaying, 0, "textures/ui/strength_effect", "diamond");
		$form->addButton("§l§bCombo\n§r§8Queued: §b".$comboqueued."§8 Matches: §b".$comboplaying, 0, "textures/items/fish_pufferfish_raw", "combo");
		$form->addButton("§l§bSumo\n§r§8Queued: §b".$sumoqueued."§8 Matches: §b".$sumoplaying, 0, "textures/ui/slow_falling_effect", "sumo");
		if($this->plugin->getDuelHandler()->isPlayerInQueue($player)){
			$result=$this->plugin->getDuelHandler()->getQueuedPlayer($player);
			if(is_null($result)) return;
			$ranked=$result->isRanked();
