	...
	*/
	public function onEntityDamageByEntity(EntityDamageEvent $event){
		...
		switch($cause){
			...
			$this->combo->set($player->getName(), 0);
            if(Utils::isPvPUtilsCounterEnabled($player)==true) $player->sendTip("§cCPS§7: §f".$damagercps."§7 | §cCombo§7: §f".$this->combo->get($damager->getName())."§7 | §cReach§7: §f".round($distance, 3));
            if(Utils::isPvPUtilsCounterEnabled($damager)==true) $damager->sendTip("§bCPS§7: §f".$damagercps."§7 | §bCombo§7: §f".$this->combo->get($damager->getName())."§7 | §bReach§7: §f".round($distance, 3));
