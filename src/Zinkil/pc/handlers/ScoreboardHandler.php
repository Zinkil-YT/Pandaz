...
class ScoreboardHandler{
	...
	public function sendMainScoreboard($player, string $title="Pandaz"):void{
		...
		$this->lineCreate($player, 0, "§f----------------- ");
		$this->lineCreate($player, 1, "§fOnline: §b".count($this->plugin->getServer()->getOnlinePlayers()));
		$this->lineCreate($player, 2, "   ");
		$this->lineCreate($player, 3, "§fYour Ping: §b".$player->getPing()."ms");
		$this->lineCreate($player, 4, "     ");
		$this->lineCreate($player, 5, "§fK: §b".$this->plugin->getDatabaseHandler()->getKills($player)." §fD: §b".$this->plugin->getDatabaseHandler()->getDeaths($player));
		$this->lineCreate($player, 6, "§fKDR: §b".$this->plugin->getDatabaseHandler()->getKdr($player)." §fElo: §b".$this->plugin->getDatabaseHandler()->getRankedElo($player));
		$this->lineCreate($player, 7, "§fKillstreak: §b".$this->plugin->getDatabaseHandler()->getKillstreak($player)." §7(".$this->plugin->getDatabaseHandler()->getBestKillstreak($player).")");
		$this->lineCreate($player, 8, "§f-----------------");
	...
	public function sendDuelScoreboard($player, string $type, string $queue, string $opponent):void{
		...
		$this->lineCreate($player, 2, "    ");
		$this->lineCreate($player, 3, "§fFighting: §c".$opponent);
		$this->lineCreate($player, 4, "§fDuration: §600:00");
	...
	public function sendPartyDuelScoreboard($player, string $queue, int $alive, int $playing):void{
		...
		$this->lineCreate($player, 2, "    ");
		$this->lineCreate($player, 3, "§fAlive: §c".$alive."/".$playing);
		$this->lineCreate($player, 4, "§fDuration: §600:00");
		$this->lineCreate($player, 5, "§f--------------------");
		$this->scoreboard[$player->getName()]=$player->getName();
		$this->duel[$player->getName()]=$player->getName();
	...
	public function sendBotDuelScoreboard($player, string $opponent):void{
		...
		$this->lineTitle($player, "§l§f» §l§bPandaz §l§f«");
		$this->lineCreate($player, 0, "§f-------------------- ");
		$this->lineCreate($player, 1, "§fFighting: §c".$opponent);
		$this->lineCreate($player, 2, "§fDuration: §600:00");
		$this->lineCreate($player, 3, "§f--------------------");
		$this->scoreboard[$player->getName()]=$player->getName();
		$this->duel[$player->getName()]=$player->getName();
	}
	public function sendDuelSpectateScoreboard($player, string $type, string $queue, string $duelplayer, string $duelopponent):void{
		...
		$this->lineCreate($player, 1, "§f".$type.": §b".$queue);
		$this->lineCreate($player, 2, "    ");
		$this->lineCreate($player, 3, "§b".$duelplayer." §6vs§c ".$duelopponent);
		$this->lineCreate($player, 0, "§f--------------------");
		$this->scoreboard[$player->getName()]=$player->getName();
	...
	public function sendPartyDuelSpectateScoreboard($player, string $queue, string $leader):void{
		...
		$this->lineCreate($player, 1, "§fParty: §b".$queue);
		$this->lineCreate($player, 2, "    ");
		$this->lineCreate($player, 3, "§6".$leader);
		$this->lineCreate($player, 0, "§f--------------------");
		$this->scoreboard[$player->getName()]=$player->getName();
		$this->spectator[$player->getName()]=$player->getName();
	}
	...
	public function updateBotDuelDuration($player, $duration){
		...
		if($this->isPlayerSetScoreboard($player)){
			if($this->isPlayerSetDuel($player)){
				...
				$this->lineCreate($player, 2, "§fDuration: §6".$duration);
			}
		}
	}
	public function updateDuelDuration($player, $duration){
		...
		if($this->isPlayerSetScoreboard($player)){
			if($this->isPlayerSetDuel($player)){
				...
				$this->lineCreate($player, 4, "§fDuration: §6".$duration);
			}
		}
	}
